...
export function renderHome(root, home, wallet, payout, actions) {
  ...
  const payoutRequired = ['withdrawal_required','sharing_required','kyc_required','kyc_pending','correction_required'].includes(payout?.journey_state);
  root.innerHTML = `<section class="page dashboard">
    <div class="welcome"><div><small>● Online — ready for today</small><h1>Welcome back, ${safe((home.profile?.name || 'Member').split(' ')[0])}</h1></div><div class="balance"><span>USD Balance</span><strong>${money.format(Number(wallet?.balances?.USD || 0) / 100)}</strong>${Number(wallet?.balances?.NGN || 0) ? `<small>${new Intl.NumberFormat('en-NG',{style:'currency',currency:'NGN',maximumFractionDigits:0}).format(wallet.balances.NGN)} legacy</small>` : ''}</div></div>
    ${payoutRequired?`<article class="withdrawal-alert"><div><span>Withdrawal journey</span><h2>${safe(payout.journey_state.replaceAll('_',' '))}</h2><p>Chat rewards are paused until the required payout step is completed.</p></div><button class="button button-primary" data-withdrawal>Continue →</button></article>`:''}
  ...
  root.querySelector('[data-primary-action]')?.addEventListener('click', () => chats[0] ? actions.openPartner(chats[0].partner_key) : root.querySelector('[data-partner]')?.scrollIntoView({ behavior: 'smooth', block: 'center' }));
  root.querySelector('[data-withdrawal]')?.addEventListener('click',actions.withdrawal);
